...
test.describe('bookstr web', () => {
  ...
  test('opens a book in the reader and can leave', async ({ page }) => {
    await page.goto('/')
    await expect(page.getByText('The Time Machine')).toBeVisible({ timeout: 30_000 })

    await page.getByRole('button', { name: /The Time Machine/ }).click()
