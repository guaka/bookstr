...
export function Settings({
  ...
  useEffect(() => {
    if (nip07) return;
    const detect = () => {
      if (typeof window.nostr?.getPublicKey === "function") setNip07(true);
    };
    const poll = window.setInterval(detect, 500);
    window.addEventListener("focus", detect);
    window.addEventListener("visibilitychange", detect);
    return () => {
      window.clearInterval(poll);
      window.removeEventListener("focus", detect);
      window.removeEventListener("visibilitychange", detect);
    };
  }, [nip07]);
    ...
    try {
      ...
      void session.done
        .then((connected) => {
          ...
          if (qrSessionRef.current === session) setQrSession(null);
        })
        .catch((e) => {
  ...
  return (
    ...
    >
      <section
        className="settings settings-panel"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <label>
          ...
          <select
            ...
            onChange={(event) =>
              onTranslationLanguage(event.target.value as TranslationLanguage)
            }
          >
        ...
        {mode === "nip46" && (
          <div className="nip07-card">
            <p>
              <strong>Connected via remote signer (NIP-46)</strong> — Amber or
              another bunker signs progress events. Your nsec never enters this
              page.
            </p>
            <div className="row">
          ...
          <>
            {nip07 ? (
              <div className="nip07-card">
                {mode === "nip07" ? (
                  <p>
                    <strong>Connected via browser extension</strong> — signing
                    uses NIP-07. Your private key never enters this page.
                  </p>
                ) : (
                  <p>
                    <strong>Browser extension detected</strong> — connect to
                    sync without pasting an nsec, or use Amber via QR below.
                  </p>
                )}
                <div className="row">
                  <button
            ...
            ) : mode === "none" && !checkingNip07 ? (
              <p className="muted">
                No NIP-07 extension found. Connect Amber with a QR code, paste a
                bunker URI, or use an nsec (advanced).
              </p>
            ) : null}
        ...
        <label>
          ...
          <textarea
            value={relays}
            onChange={(e) => setRelaysField(e.target.value)}
            rows={4}
          />
        </label>
        <button
